...
//추천 여부 찾기
export const getUserRecommendation = async (menu_id: number) => {
  try {
    const response = await axiosInstance.get(`recommend/${menu_id}/`);
    return response.data; // 추천 여부 데이터 반환
  } catch (error) {
